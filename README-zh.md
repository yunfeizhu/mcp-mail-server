...
| | |
|---|---|
| **🔎 快速找到重要邮件**<br>跨文件夹按发件人、收件人、主题、正文、日期、已读状态和回复状态搜索。 | **✉️ 在对话中直接处理**<br>用自然语言阅读、发送、回复、移动和删除邮件。 |
| **📎 完整处理附件**<br>查看附件信息、下载文件，并通过明确的目录白名单发送本地附件。 | **🔐 数据由你掌控**<br>服务在本地运行，直连邮箱服务商，同时启用证书校验和载荷大小限制。 |
